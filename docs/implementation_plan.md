...
- [ ] **Classe `Combo`** — nome (`str`) e descrição/sequência de técnicas (`str`).
  - [ ] Validação: nome não vazio.
- [ ] **Classe `FootworkMove`** — nome da movimentação (`str`).
  - [ ] Validação: nome não vazio.
- [ ] **Classe `CustomWorkout`** — nome (`str`), duração estimada (`int`, segundos) e descrição opcional (`str`).
  - [ ] Validação conforme **RN04**: nome não vazio e duração > 0.
- [ ] **Classe abstrata `DrillConfig`** — configuração base de qualquer modalidade (duração total, etc.).
- [ ] **Classe `RoundTimerConfig(DrillConfig)`** — número de rounds, duração trabalho, duração descanso, tempo aviso.
- [ ] **Classe `TimingDrillConfig(DrillConfig)`** — duração total, intervalo mín, intervalo máx, técnica-alvo.
  - [ ] Validação conforme **RN01**: intervalo mín < intervalo máx.
- [ ] **Classe `ComboDrillConfig(DrillConfig)`** — lista de `Combo` selecionados, modo de chamada (enum `SEQUENTIAL`/`RANDOM`), intervalo entre chamadas, duração total.
  - [ ] Validação: pelo menos um combo selecionado.
- [ ] **Classe `FootworkDrillConfig(DrillConfig)`** — lista de `FootworkMove` selecionados, intervalo mín, intervalo máx, duração total.
  - [ ] Validação conforme **RN01** e **RN05**: intervalo mín < intervalo máx; pelo menos uma movimentação selecionada.
- [ ] **Enum `CallMode`** — `SEQUENTIAL`, `RANDOM`.
- [ ] **Classe `ComboLibrary`** — coleção de `Combo` com métodos CRUD (add, update, remove, list, get_by_name).
- [ ] **Classe `FootworkMoveLibrary`** — coleção de `FootworkMove` com métodos CRUD.
- [ ] **Classe `CustomWorkoutLibrary`** — coleção de `CustomWorkout` com métodos CRUD.
- [ ] Testes unitários para todas as classes e validações desta fase.
